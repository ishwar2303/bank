<?php 
    ...
    if(isset($_GET['status_id'])){
        ...
        $sql = "SELECT * FROM car_loan_status WHERE status_id = '$status_id'";
        $result = $conn->query($sql);
        if($conn->error == ''){
            if($result->num_rows == 1){ 
                $status = $result->fetch_assoc();
                $case_id = $status['case_id'];
                //$amount_recovered = $status['amount_recovered'];
                $bill_raised = $status['bill_raised'];
                $payment_received = $status['payment_received'];
                $payment_received_on = $status['payment_received_on'];
                $auction_date = $status['auction_date'];
                $auction_amount = $status['auction_amount'];
                $recovery_date = $status['recovery_date'];
                $full_amount = $status['full_amount'];
                $part_amount = $status['part_amount'];
                $regularise_date = $status['regularise_date'];
                $full_payment_paid_on = $status['full_payment_paid_on'];
            }
            else{
    ...
    if(isset($_POST['billRaised']) && isset($_POST['paymentReceived']) && isset($_POST['paymentReceivedOn']) && isset($_POST['auctionDate']) && isset($_POST['auctionAmount']) && isset($_POST['recoveryDate']) && isset($_POST['fullAmount']) && isset($_POST['partAmount']) && isset($_POST['regulariseDate'])){
        ...
        if($control){ // Insert data into database control = 1
            ...
            $sql = "UPDATE car_loan_status SET 
            ...
            full_payment_paid_on = '$full_payment_paid_on' 
            WHERE status_id = '$status_id'";
            
            if($conn->query($sql) === TRUE){ 
                $sql = "INSERT INTO `user_activity` (`activity_id`, `loan`, `case_id`, `user_id`, `operation_id`, `timestamp`) VALUES (NULL, '2', '$case_id', '$_SESSION[user_id]', '4', '$timestamp')";
                $conn->query($sql);
